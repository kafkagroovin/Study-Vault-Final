...
function Study({sessions,onAdd,apiKey,t}){
  ...
  const analyze=async()=>{
    ...
    const r=await ai("Analyze student study sessions. Give 3 bullet insights with emojis. No asterisks or markdown. Sessions: "+data,apiKey);
...
function Money({transactions,budget,onAddTxn,onDelTxn,onSetBudget,apiKey,t}){
  ...
  const doRoast=async()=>{
    ...
    const r=await ai(`Roast this student's spending in 3 funny sentences then give 2 real tips. No asterisks or markdown. Budget: ${c}${budget.monthly||"unknown"} Spending: ${data}`,apiKey);
...
function NoteEditor({note,onSave,onClose,apiKey,t}){
  ...
  const doAi=async action=>{
    ...
    try{
      if(action==="summarize"){const r=await ai("Summarize in 5 bullet points starting with •. No asterisks or markdown:\n\n"+txt,apiKey);setAiPanel({content:r});}
      else if(action==="explain"){const r=await ai("Explain simply for a 16-year-old. No asterisks or markdown:\n\n"+txt.slice(0,1500),apiKey);setAiPanel({content:r});}
      else if(action==="expand"){const r=await ai("Expand with more detail. No asterisks or markdown:\n\n"+txt.slice(0,1500),apiKey);if(edRef.current)edRef.current.innerHTML+=`<br><hr style='border:none;border-top:1px solid rgba(255,255,255,0.1);margin:14px 0'><strong style='color:${t.acc2}'>✦ Expanded</strong><br><br>`+r.replace(/\n/g,"<br>");autoSave();toast("Expanded ✅");}
      else if(action==="grammar"){const r=await ai("Fix grammar only. Return ONLY corrected text. No explanations:\n\n"+txt,apiKey);if(edRef.current)edRef.current.innerHTML=r.replace(/\n/g,"<br>");autoSave();toast("Fixed ✓");}
      else if(action==="quiz"){const raw=await ai('Create 5 MCQs. Return ONLY valid JSON array with no extra text: [{"q":"...","options":["A","B","C","D"],"answer":0}]\n\nContent:\n'+txt.slice(0,2000),apiKey);
        try {
          const qs=JSON.parse(raw.replace(/```json|```/g,"").trim());
          setQuiz({qs, i:0, score:0, sel:null, done:false, answered:false});
        } catch(e) {
          toast("Failed to parse quiz. Try again.");
        }
      }
      else if(action==="flashcards"){const raw=await ai('Create 6 flashcards. Return ONLY valid JSON array with no extra text: [{"front":"term","back":"definition"}]\n\nContent:\n'+txt.slice(0,2000),apiKey);
        try {
          const cards=JSON.parse(raw.replace(/```json|```/g,"").trim());
          setFc(cards);setFcI(0);setFcFlip(false);
        } catch(e) {
          toast("Failed to parse flashcards. Try again.");
        }
      }
    } catch(error) {
      console.error("AI error:", error);
      toast("AI error — check your Gemini key in Settings");
    }
    setAiLoad(false);
  };
